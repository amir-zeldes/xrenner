...
class Xrenner:
    ...
    def analyze(self, infile, out_format):
        ...
        self.conll_tokens.append(
            ParsedToken(0, "ROOT", "--", "XX", "", -1, "NONE", Sentence(1, 0, ""), [], [], [], self.lex))
        self.markables = []
            ...
            if "speaker" in myline and "=" in myline and myline.startswith("#"):  # speaker annotation
                ...
                if cols[3] in lex.func_substitutes_forward and int(cols[6]) > int(cols[0]):
                    tok_func = re.sub(lex.func_substitutes_forward[cols[3]][0],
                                      lex.func_substitutes_forward[cols[3]][1], cols[7])
                elif cols[3] in lex.func_substitutes_backward and int(cols[6]) < int(cols[0]):
                    tok_func = re.sub(lex.func_substitutes_backward[cols[3]][0],
                ...
                conll_tokens.append(ParsedToken(str(int(cols[0]) + self.tokoffset), cols[1], cols[2], cols[3], cols[5],
                                                head_id, tok_func, current_sentence, [], [], [], lex, quoted, cols[8],
                                                cols[9]))
                self.sentlength += 1
                # Check not to add a child if this is a function which discontinues the markable span
                if not (lex.filters["non_link_func"].match(tok_func) is not None or lex.filters["non_link_tok"].match(
                        cols[1]) is not None):
        ...
        if lex.filters["seek_verb_for_defs"]:
            for mark in self.markables:
                if mark.definiteness == "def" and mark.antecedent == "none" and mark.form == "common" and \
                        (lex.filters["event_def_entity"] == mark.entity or lex.filters[
                            "abstract_def_entity"] == mark.entity):
                    for tok in conll_tokens[0:mark.start]:
                        if lex.filters["verb_head_pos"].match(tok.pos):
                            if stems_compatible(tok, mark.head, lex):
                                v_antecedent = make_markable(tok, conll_tokens, {}, tok.sentence.start_offset,
                                                             tok.sentence, [], lex)
    ...
    def analyze_markable(self, mark, lex):
        ...
        if mark.agree in lex.filters["agree_entity_mapping"]:
            ...
            resolve_mark_entity_classify(mark, lex)
    ...
    def process_sentence(self, tokoffset, sentence):
        ...
        for tok1 in conll_tokens[tokoffset + 1:]:
            ...
            tok1.sent_position = float(int(
                tok1.id) - tokoffset) / sentence.token_count  # Add relative token positions at sentence as percentages
            tok1.doc_position = float(
                int(tok1.id)) / self.token_count  # Add relative token positions at document as percentages
        ...
        for token in conll_tokens[tokoffset:]:
            ...
            if lex.filters["possessive_func"].match(token.func) is not None:
                ...
                if lex.filters["pronoun_pos"].match(token.pos) is None and lex.filters["pronoun_pos"].match(
                        conll_tokens[int(token.head)].pos) is None:
                    lex.hasa[token.text][conll_tokens[int(
                        token.head)].text] += 2  # Increase by 2: 1 for attestation, 1 for pertinence in this document
            ...
            if token.func2 != "_":
                if lex.filters["possessive_func"].match(token.func2) is not None:
                    if lex.filters["pronoun_pos"].match(token.pos) is None and lex.filters["pronoun_pos"].match(
                            conll_tokens[int(token.head2) + tokoffset].pos) is None:
                        lex.hasa[token.text][conll_tokens[int(
                            token.head2) + tokoffset].text] += 2  # Increase by 2: 1 for attestation, 1 for pertinence in this document
        ...
        for tok1 in conll_tokens[tokoffset + 1:]:
            ...
            if tok1.text == lex.debug["ana"]:
                pass
            if lex.filters["postprocess_parser"]:
                if ((lex.filters["mark_head_pos"].match(tok1.pos) is not None and lex.filters[
                    "mark_forbidden_func"].match(tok1.func) is None) or
                    ...
                    if tok1.text.strip() in lex.affix_tokens:
                        ...
                        for child_id in sorted(children[tok1.id], reverse=True):
                            ...
                            if ((lex.filters["mark_head_pos"].match(child.pos) is not None and lex.filters[
                                "mark_forbidden_func"].match(child.func) is None) or
                                pos_func_combo(child.pos, child.func, lex.filters["pos_func_heads"])) and not (
                            stop_ids[child.id]):
                                ...
                                for tok_to_rewire in conll_tokens[tokoffset + 1:]:
                                    if tok_to_rewire.original_head == tok1.id and tok_to_rewire.head != child.id and tok_to_rewire.id != child.id:
                                        ...
                                        # Rewire modifiers
                                        if tok_to_rewire not in child.modifiers and lex.filters["mod_func"].match(
                                                tok_to_rewire.func) is not None:
        ...
        for tok1 in conll_tokens[tokoffset + 1:-2]:
            ...
            first_name_candidate = tok1.text.title() if tok1.text.isupper() else tok1.text
            middle_name_candidate = conll_tokens[int(tok1.id) + 1].text.title() if tok1.text.isupper() else \
            conll_tokens[int(tok1.id) + 1].text
            last_name_candidate = tok2.text.title() if tok2.text.isupper() else tok2.text
            if first_name_candidate in lex.first_names and last_name_candidate in lex.last_names and tok1.head == tok2.id and (
                    re.match(r'^[A-Z]\.$', middle_name_candidate) or middle_name_candidate in lex.first_names):
                stop_ids[tok1.id] = True
        ...
        for tok in conll_tokens[tokoffset + 1:]:
            ...
            if ((lex.filters["mark_head_pos"].match(tok.pos) is not None and lex.filters["mark_forbidden_func"].match(
                    tok.func) is None) or
                ...
                if make_submark:
                    ...
                    small_markable = make_markable(tok, conll_tokens, descendants, tokoffset, sentence, keys_to_pop,
                                                   lex)
        ...
        for mark_id in mark_candidates_by_head:
            ...
            if mark.end > mark.start:  # No atomicity check if single token
                ...
                if (is_atomic(mark, lex.atoms, lex) or (
                        "@" in modifier_based_entity and "_" not in mark_id)) and mark.end > mark.start:
                    for index in enumerate(mark_candidates_by_head):
                        ...
                        if key != mark.head.id and mark.start <= int(
                                re.sub('_.*', '', key)) <= mark.end and '_' not in key:
                            if lex.filters["pronoun_pos"].match(conll_tokens[int(
                                    re.sub('_.*', '', key))].pos) is None:  # Make sure we're not removing a pronoun
        ...
        for mark_id in mark_candidates_by_head:
            ...
            this_markable = Markable("referent_" + str(self.markcounter), mark.head, mark.form,
                                     mark.definiteness, mark.start, mark.end, mark.text, mark.core_text, mark.entity,
                                     mark.entity_certainty,
                                     ...
                                     mark.submarks, mark.coordinate, mark.agree_certainty)
        ...
        for current_markable in markables[processed_marks:]:
            ...
            if antecedent_prohibited(current_markable, conll_tokens, lex) or (
                    current_markable.definiteness == "indef" and lex.filters["apposition_func"].match(
                    current_markable.head.func) is None and not lex.filters["allow_indef_anaphor"]):
                antecedent = None
            elif (current_markable.definiteness == "indef" and lex.filters["apposition_func"].match(
                    current_markable.head.func) is not None and not lex.filters["allow_indef_anaphor"]):
